...
connectionRequestRouter.post('/sendConnectionRequest/send/:status/:receiverId', userAuthMiddleware, async (req, res) => {
    try {
        const senderId = req.user.id;
        const receiverId = req.params.receiverId;
        const status = req.params.status;
        if (!senderId || !receiverId || !status) {
            return res.status(400).send("Please provide senderId, receiverId and status");
        }
